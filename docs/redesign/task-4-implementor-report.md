...
- **Correction (2026-08-11, post-review):** this section originally claimed "no paraphrasing, no re-derivation" for every copy string and alt text. That claim was inaccurate and has been corrected below after an independent Task 4 review caught it.
- Every nav href, anchor ID, and image filename was copied verbatim from `content-contract.md` / `page-spec.md`'s tables - no paraphrasing, no re-derivation there.
- **Three portfolio `alt` strings were corrected, not copied verbatim, from `flowers-catalog.json`'s AI-generated `alt_text_cs` field**, because the catalog values contain small Czech-language errors (a garbled word, a missing háček, a non-standard adjective). This is documented in `content-contract.md`'s new "4. Alt-text corrections (catalog typos)" section, following the exact same pattern the plan already used for the two mockup-copy typo corrections (Section 1 of `content-contract.md`): `18ef0981-...jpeg` (`osusíchlým` → `osušeným`), `7f8f3457-...jpeg` (`doplnky` → `doplňky`), `74d78256-...jpeg` (`Rustikový` → `Rustikální`). Every other alt attribute on the page (hero, category cards, about photo, and the other 3 portfolio images) is copied character-for-character from `page-spec.md`'s image table / the catalog, unaltered.
- Portfolio `<img>` tags also carry a `rounded-sm` class not listed in `page-spec.md` §2.5's prescribed `class="w-full aspect-square object-cover"`. This was an undisclosed addition, flagged by the same review - harmless and consistent with the near-rectangular-but-not-sharp corner treatment used everywhere else on the page (hero photo, about photo, card photos, buttons all use `rounded-sm`), but it should have been called out here originally. No code change made in response since the reviewer confirmed it's fine as-is.
